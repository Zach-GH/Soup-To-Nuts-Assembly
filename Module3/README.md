...
I attempted a re-install of Debian 10 instead of raspberrian 11 using my VM, however I ended with the same result. I am now thinking that perhaps I need to start investigating the actual gcc compiler versions being used, or look into some other potential reasons that I could be missing a dependency, or having an updated version of the language I am attempting to run. This at least crosses of the idea that it could be the version of the OS that I am using, as I have used two completely different versions and am getting the same outcome.

For the sake of recording which GCC version I am using on my school provided server, I ran `gcc --version` and got `gcc (Raspbian 8.3.0-6+rpi1) 8.3.0` For experimental purposes, I found a 32 Bit installation image that I can install onto my mac's version of vmware, I would imagine that this otherwise would not be able to work because my mac is a x64 bit OS, however, the fact that the raspberrian image is x32 peaks my interest to see if I would get the same output as my windows OS.
